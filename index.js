...
const chainId = 88
const pkey = '0d5bb1d7a6d2694bf548d195b8cf4ed8e98b716df14c55d3dfa83229053b546f' // token holder pkey
const contractAddress = '0x45da16b69d777677fD384C8920Ac0cf8F5FE884b' // token contract address
...
if(ctx.message.chat.type != 'private'){
  ...
  let dbData = await db.collection('allUsers').find({userId: ctx.from.id}).toArray()
   ...
   Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change '+data.currency+'/TomoChain Wallet', 'iamsetemail')]
      ])
      )  
       .catch((err) => sendError(err, ctx))
    }else{
ctx.replyWithMarkdown('💡 *Your TomoChain Wallet is:* _not set_', 
    Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change '+data.currency+'/TomoChain Wallet', 'iamsetemail')]
      ])
      ) 
...
if(check.length===0){
ctx.replyWithMarkdown(
'🖊* Done:* Your TomoChain Address is\n`'+ctx.message.text+'`',
...
ctx.replyWithMarkdown("❌ *You have to own at least "+min_wd.toFixed(8)+" "+bot_cur+" in your balance to withdraw!*")
    ...
    ctx.replyWithMarkdown('💡 *Your TomoChain wallet addres:* `not set`', 
    Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change YouR '+data.currency+'/TomoChain Wallet', 'iamsetemail')]
      ])
      ) 
           .catch((err) => sendError(err, ctx))
...
ctx.replyWithMarkdown('*Your Withdraw Is Instantly Paid To Your Tomo Wallet ✔️*\n\n*💳 Transaction Details *\n*💵 Amount* : '+msg+' CINU\n*🏦 Wallet* : '+wallet+'\n*🏧 Transaction Link :*['+result.transactionHash+'](https://tomoscan.io/tx/'+result.transactionHash+''),

ctx.replyWithMarkdown(`*ℹ️ Add the contract address to see the balance in your wallet*

*🔹 Contract address:* 0x45da16b69d777677fD384C8920Ac0cf8F5FE884b

*🔹 Network:* TomoChain`),

bot.telegram.sendMessage(data.payment_channel,

'*New Withdraw Is Instantly Paid to Tomo Wallet ✔️*\n*🤖 BOT LINK:* @CrazyInuAirdropBot\n\n*💳 Transaction Details *\n*💵 Amount* : '+msg+' CINU\n*🏦 Wallet* : '+wallet+'\n*🏧 Transaction Link :*['+result.transactionHash+'](https://tomoscan.io/tx/'+result.transactionHash+'',{parse_mode: 'Markdown',disable_webpage_preview:true}).catch((err) => sendError(err, ctx))
...
function mustJoin(ctx){
  let msg = '*🔎 Join Our All Channels:*\n➖➖➖➖➖➖➖➖➖➖➖*'
  for(var ind in channels){
  var cha = channels[ind]
  msg+='\n🔹'+cha
  }
  msg+='*\n➖➖➖➖➖➖➖➖➖➖➖\n⛱*Payment Channel*\n*'+data.payment_channel+'*\n➖➖➖➖➖➖➖➖➖➖➖\n▪️ Follow *DynamicAirdrops* [Twitter](http://twitter.com/dynamicairdrops)\n▪️ Follow *AirdropKart* [Twitter](https://twitter.com/AirdropKart)\n▪️ Follow *DefiClan* [Twitter](http://twitter.com/Defi_clan)\n\n*🛃 Before Using This Bot!*'
  ctx.replyWithMarkdown(msg, {disable_web_page_preview: 'true',reply_markup: { keyboard: [['🟢 Joined']], resize_keyboard: true
 }
 })
 }
...
bot.hears('🟢 Joined', ctx => {
  var kkc = `🔹 Join *Legit Airdrops* [Telegram Channel](https://t.me/legitairdrops010) And *Comment On the recent post*
  ーーーーーーーーーーーー
...
bot.hears('✅ Done', ctx => {
  var kkc = `🔹 Follow *Dynamic Airdrops* on [Twitter](https://mobile.twitter.com/DynamicAirdrops) And [Telegram](https://t.me/dynamicairdrops)
ーーーーーーーーーーーー
